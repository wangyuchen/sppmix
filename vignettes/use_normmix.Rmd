...
You may also use a `normmix` object and supply the other information to sppmix. Truncation is another feature of `rsppmix()`. When `truncate` is true (default), the simulated points are guranteened to be within the window When it's false, a warning will be given when there are points outside window.

```{r}
rsppmix(mix1, truncate = FALSE, lambda = 100, win = square(1))
```
