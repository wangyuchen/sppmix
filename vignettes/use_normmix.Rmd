...
It prints out the $\mu$'s but still not the $\Sigma$'s. You can also use ```is.normmix``` to test if an object is a valid normal mixture.

## Create Intensity surface
A normal mixture is just a step away from an intensity surface. In most 
situations, you may have a intensity value $lambda$ associated with the normal
mixture. An intensity value indicates how many points the normal mixture will 
have on average if a domain is defined. 
